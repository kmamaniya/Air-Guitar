...
bool Adafruit_ADXL343:: disableInterrupt() {
    writeRegister(ADXL343_REG_INT_ENABLE, 0x00);
    return true;
}

bool Adafruit_ADXL343:: disableFF() {
    writeRegister(ADXL343_REG_FIFO_CTL, 0x00);
    return true;
}

bool Adafruit_ADXL343:: setTHRESHOLD(int value) {
    writeRegister(ADXL343_REG_THRESH_TAP, (uint8_t)value);
    writeRegister(ADXL343_REG_THRESH_ACT, (uint8_t)255);
    writeRegister(ADXL343_REG_THRESH_INACT, (uint8_t)255);
    writeRegister(ADXL343_REG_THRESH_FF, (uint8_t)255);
    writeRegister(ADXL343_REG_TIME_FF, (uint8_t)255);
    return true;
}

bool Adafruit_ADXL343:: setLatency(int value) {
    writeRegister(ADXL343_REG_LATENT, (uint8_t)value);
    return true;
}

bool Adafruit_ADXL343:: setDuration(int value) {
    writeRegister(ADXL343_REG_DUR, (uint8_t)value);
    return true;
}

int Adafruit_ADXL343:: enableAxes(bool s,bool x_axis, bool y_axis, bool z_axis) {
    uint8_t value = (s<<3|(x_axis)<<2 | (y_axis)<<1 | z_axis);
    writeRegister(ADXL343_REG_TAP_AXES, (uint8_t)value);
    return value;
}
